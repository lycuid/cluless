...
static inline void create_workspace_section(Monitor *mon)
{
...
static inline void create_layout_section(Monitor *mon)
{
...
static inline void create_title_section(Monitor *mon)
{
  ...
  if (active) {
    ...
    if (core->get_window_title(active->window, &wm_name) && wm_name.nitems) {
      ...
      sprintf(title_section,
              LogFormat[FmtWindowTitle] ? LogFormat[FmtWindowTitle] : "%s",
              title);
    }
  }
}

void log_statuslog(Monitor *mon)
{
...
#define AppendSection(string)                                                  \
  ...
  create_workspace_section(mon);
  AppendSection(workspace_section);
  AppendSeperator();

  create_layout_section(mon);
  AppendSection(layout_section);
  AppendSeperator();

  create_title_section(mon);
